...
{
	static float alpha_=0.02f;
	static float bearing_filter;
	static float delta_f;
	static float bearing_last;
	delta_f = fabs(bearing_last-bearing);
	bearing_filter = alpha_*delta_f + (1.0f -  alpha_)*bearing;
	bearing_last = bearing;
	return bearing_filter;
}
